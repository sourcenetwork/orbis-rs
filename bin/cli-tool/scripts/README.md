...
Each run creates a fresh ring, DKG ceremony, stored secret, ACP object/relationships, and key derivation on-chain (both ACP policies are pre-existing, passed in — the script never creates a policy). There is currently no cleanup command in the CLI, so every run leaves permanent artifacts on the target chain — an accepted cost of testing the real create-then-use flow, not something this script attempts to reverse.
...
- **A funded `ORBIS_SIGNING_KEY`.** Every write this script performs (`create-ring`, `register-object-to-chain`, `set-relationship-on-chain`, `post-key-derivation`) only needs ordinary funded-account rights. It does **not** need node-controller privileges — this script never calls `add-node-to-whitelist`/`update-node-peer-id`.
...
- **An existing object/secret-access ACP policy_id.** `--object-policy-id`/`ORBIS_OBJECT_POLICY_ID` must already exist on the target chain — the script never creates one, only uses it for `register-object-to-chain`/`set-relationship-on-chain`/`store-secret`/`post-key-derivation`. Its schema must support resource `$RESOURCE` (default `document`) with a `$RELATION` relation (default `reader`) grantable to an actor, and a `$PERMISSION` permission (default `read`) satisfied by that relation. The simplest way to get one: run `cli-tool add-policy-to-chain` once (it creates exactly this schema) and reuse the printed `POLICY_ID` across runs.
...
Note there are two different, unrelated policy IDs in this flow, both pre-provisioned, both required:
- `ORBIS_WHITELIST_POLICY_ID` — governs which nodes may participate in DKG for rings created under it (checked against each node's `NodeInfo.whitelisted_policy_ids`).
- `ORBIS_OBJECT_POLICY_ID` — governs ACP read/write access to the stored secret and key derivation objects this script creates. Unrelated schema, unrelated purpose — don't assume these are (or should be) the same policy.
...
./remote_smoke_test.sh \
  ...
  --object-policy-id "$KNOWN_OBJECT_POLICY_ID" \
...
| `--whitelist-policy-id <id>` | `ORBIS_WHITELIST_POLICY_ID` | none | **yes** |
| `--object-policy-id <id>` | `ORBIS_OBJECT_POLICY_ID` | none | **yes** |
| `--peer-node-keys <csv>` | `ORBIS_SMOKE_PEER_NODE_KEYS` | none | **yes** |
| `--threshold <n>` | `ORBIS_SMOKE_THRESHOLD` | none | **yes** |
...
| `--resource`/`--permission`/`--relation` | `ORBIS_SMOKE_RESOURCE`/`_PERMISSION`/`_RELATION` | `document`/`read`/`reader` | no — must match the ACP schema of `--object-policy-id` |
