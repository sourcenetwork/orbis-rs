...
Each run creates a fresh ACP policy, ring, DKG ceremony, stored secret, and key derivation on-chain. There is currently no cleanup command in the CLI, so every run leaves permanent artifacts on the target chain — an accepted cost of testing the real create-then-use flow, not something this script attempts to reverse.
...
- **A funded `ORBIS_SIGNING_KEY`.** Every write this script performs (`add-policy-to-chain`, `create-ring`, `register-object-to-chain`, `set-relationship-on-chain`, `post-key-derivation`) only needs ordinary funded-account rights. It does **not** need node-controller privileges — this script never calls `add-node-to-whitelist`/`update-node-peer-id`.
...
Note there are two different policy IDs in this flow, with two different lifetimes:
- `ORBIS_WHITELIST_POLICY_ID` — must already exist and be pre-provisioned on the target network; the script only reads it, never creates it.
- The object/secret-access policy — created fresh every run via `add-policy-to-chain` (a fixed built-in policy body, resource `document`, relations `creator`/`reader`, permission `read = creator + reader`; unconditionally safe to create, no gating). Not something you configure.
...
| `--resource`/`--permission`/`--relation` | `ORBIS_SMOKE_RESOURCE`/`_PERMISSION`/`_RELATION` | `document`/`read`/`reader` | no — must match the fixed policy schema `add-policy-to-chain` creates |
