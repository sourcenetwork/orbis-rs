...
///
/// Serialized as a compact binary format (no JSON) so the plaintext buffer can
/// be held in a `Zeroizing` wrapper throughout the write path.
///
/// Wire format (all lengths are 4-byte little-endian u32):
/// ```text
/// [1B version = 0x01]
/// [4B: share_bytes length][N bytes: share_bytes]
/// [4B: public_polynomial utf-8 length][M bytes: public_polynomial]
/// [8B LE u64: last_pss]
/// ```
#[derive(Clone, Debug)]
