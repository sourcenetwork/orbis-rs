...
use proto::unsafe_testing::{
    ...
    SubmitOrganicInvalidManifestRequest, SubmitOrganicInvalidRefreshResultRequest,
    SubmitOrganicNoncanonicalPrepareRequest, SubmitPssStallOfflineReportRequest,
    SubmitUnauthorizedRelayEvidenceRequest,
};
...
/// Exercises `dkg_leader_public_fault`/`InvalidManifest` detection: node1 (the
/// real canonical refresh leader) uses `SubmitOrganicInvalidManifest` to
/// broadcast a first, standalone Phase1-Commitments manifest that drops one
/// real retained contribution before recomputing `phase_root` — internally
/// self-consistent (real signed data, correctly-recomputed root), but naming
/// an incomplete origin set for a phase that requires the full committee.
/// Node2/node3's *unmodified* `PublicBatchAssembler::insert_manifest`
/// (`network.rs`) rejects it and reports node1, rather than evidence being
/// injected directly.
///
/// Unlike `test_refresh_organic_invalid_result_triggers_on_chain_report`
/// (#9), this cannot wait for the ceremony to complete first: by the time a
/// real, complete manifest has already been claimed for this phase, a later
/// differently-rooted manifest would hit `claim_phase_root`'s cross-root
/// check and be attributed as `dkg_leader_equivocation` instead of the
/// `InvalidManifest` case this test targets (see `network.rs`'s
/// `CompletePhaseRootClaim`). So this retries the RPC every 250ms starting
/// right after session-active, firing it as soon as the server reports at
/// least 2 retained contributions — the earliest possible moment, racing
/// node1's own automatic completion (which needs all 3). A timing
/// heuristic, not a hard guarantee, matching this file's other organic
/// tests; if the real manifest completes first, the RPC either fails
/// loudly (its own `at least 2 retained contributions` precondition can
/// still pass but the resulting fault would misattribute as equivocation)
/// so a flake here is loud, not silent.
#[tokio::test]
#[serial_test::serial]
async fn test_refresh_organic_invalid_manifest_triggers_on_chain_report() {
    println!("Starting PSS refresh organic invalid-manifest reporting integration test...");
    ...
    println!(
        "Racing node1's own automatic Phase1 completion: retrying \
         SubmitOrganicInvalidManifest every 250ms until at least 2 real \
    ...
    loop {
        match node1_unsafe_client
            .submit_organic_invalid_manifest(SubmitOrganicInvalidManifestRequest {
            ...
            .await
        {
            ...
            Err(error) => {
                assert!(
                    Instant::now() < deadline,
                    "node1 never had enough retained contributions to broadcast an \
    ...
    println!("Waiting for organic invalid-manifest EventReportAccepted on chain (up to 120s)...");
    let event = sub
        .wait_for_report_accepted_matching(RING_ID, Duration::from_secs(120), |event| {
            event.report_type == "invalid_crypto_response" && event.accused_node_key == NODE_KEY_1
        })
        .await
        .expect("organic invalid-manifest EventReportAccepted should be emitted");

    println!(
        "Organic invalid-manifest report accepted on chain: report_id={} accused={} reporter={}",
        event.report_id, event.accused_node_key, event.reporter_node_key
    );
    ...
    assert_eq!(
        event.accused_node_key, NODE_KEY_1,
        "node1 (leader) should be the accused for broadcasting an invalid manifest"
    );
    assert_eq!(event.ring_id, RING_ID, "ring_id mismatch");
    assert!(
        [NODE_KEY_2, NODE_KEY_3].contains(&event.reporter_node_key.as_str()),
