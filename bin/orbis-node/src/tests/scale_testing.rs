...
//! shared in-memory [`DummyBulletin`] — no Docker, no chain. Uses the same
//! `crate::harness` building blocks as orbis-bench's in-process backend
//! (`bind_addr_v4("127.0.0.1:0")` to avoid iroh magicsock contention at this
//! scale — see that module's docs), driven directly here so this can run in
//! CI without pulling in the orbis-bench binary. PRE/SIGN reuse `cli_tool`'s
//! own request-building/decrypt/verify helpers (already a dev-dependency)
...
#[tokio::test]
async fn test_scale_dkg_pre_sign_25_nodes() {
    ...
    // All 25 iroh endpoints just bound in quick succession; give magicsock's
