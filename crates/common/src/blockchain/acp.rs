...
impl Policy {
    /// Get the relations that grant a permission for a given resource.
    ///
    /// Parses the permission expression and extracts relation names.
    ...
    pub fn get_relations_for_permission(&self, resource: &str, permission: &str) -> Option<Vec<String>> {
        ...
        // Find the permission
        let perm = res.permissions.iter().find(|p| p.name == permission)?;

        // Parse the expression to extract relations
        // For now, we support simple union expressions like "owner + reader"
        let relations = Self::parse_permission_expr(&perm.expression);

        Some(relations)
    }

    /// Parse a permission expression and extract relation names.
    ///
    /// Supports:
    /// - Simple relations: "owner"
    /// - Union (OR): "owner + reader" or "(owner + reader)"
    /// - Intersection (AND): "owner & admin" (both required - returns both for checking)
    ///
    /// Note: Arrow expressions like "owner->member" are more complex and
    /// require recursive relationship checking; for now we return them as-is.
    fn parse_permission_expr(expr: &str) -> Vec<String> {
        ...
        // Handle union expressions (most common)
        if expr.contains('+') {
            return expr
                .split('+')
                .map(|s| Self::clean_relation_name(s))
                .filter(|s| !s.is_empty())
                .collect();
        }

        // Handle intersection expressions
        if expr.contains('&') {
            return expr
                .split('&')
                .map(|s| Self::clean_relation_name(s))
        ...
        vec![Self::clean_relation_name(expr)]
    }

    /// Clean a relation name by removing parentheses and whitespace.
    fn clean_relation_name(s: &str) -> String {
        s.trim()
            .trim_matches(|c| c == '(' || c == ')')
            .trim()
            .to_string()
