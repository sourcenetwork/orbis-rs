    ...
    RD: Fn(usize, usize) -> Result<(PK, Vec<PriShare<SV>>, PP)>,
{
    ...
    let (agg_pk, shares, pub_poly) = run_dkg(n, t)?;
    let derivation: &[u8] = b"test-sign-derivation-v1";
    let metadata: &[u8] = &T::encode_metadata("policy-id", "read", Some(0u64));
    ...
    RD: Fn(usize, usize) -> Result<(PK, Vec<PriShare<SV>>, PP)>,
{
    ...
    let (_agg_pk, shares, pub_poly) = run_dkg(n, t)?;
    let derivation: &[u8] = b"test-sign-derivation-v1";
    let metadata: &[u8] = &T::encode_metadata("policy-id", "read", None);
    let wrong_metadata: &[u8] = &T::encode_metadata("policy-id_wrong", "read_wrong", Some(0u64));
